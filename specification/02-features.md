...
- **A household edits the names of its own people**, added and seeded alike (`F4-B04`, decided 2026-09-20). We type the guest list off an address book; the people on it know how they are called, and "Erika Huber" who signs everything "Oma Erika" should not need a phone call to get her place card right. The name we invited them under is kept in `guest.seeded_name` and shown to us as "Eingeladen als …", so a rename never costs us the link to the posted card. Rejected: restricting the edit to guest-added members — the case that actually occurs is a seeded name we got slightly wrong — and a rename endpoint of its own, since the name saves with the rest of the form.
