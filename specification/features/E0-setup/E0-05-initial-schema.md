...
- `0001-initial-schema.sql` creating every table in [03-data-model](../../03-data-model.md): `household`, `guest`, `seating_unit`, `seat`, `seat_assignment`, `budget_item`, `photo`, `session`, `audit_log`, `app_setting`.
...
2. `CHECK` constraints on every enum column, with the **English** values: `guest.attending`, `guest.kind`, `guest.origin`, `guest.meal_choice`, `guest.portion`, `guest.seating_need`, `seating_unit.venue`, `budget_item.status`, `session.subject_type`, `audit_log.actor_type`, `audit_log.action`.
...
4. Foreign keys with explicit behaviour: `guest.household_id` → `ON DELETE CASCADE`; `seat_assignment.guest_id` → `ON DELETE CASCADE`; `seat.seating_unit_id` → `ON DELETE CASCADE`; `seat_assignment.seat_id` → `ON DELETE RESTRICT`, because deleting a seat — or a unit whose seats cascade — that still seats people should be refused, not silently accepted.
   The `venue` column is carried down `seating_unit → seat → seat_assignment` as a composite foreign key, which is what makes `UNIQUE (guest_id, venue)` express "one seat per guest per venue" without a trigger. See the data model for why the denormalisation cannot diverge.
5. Defaults per the data model: `portion` = `'full'`, `seating_need` = `'normal'`, `midnight_snack` = 0, transport seats = 0.
6. Indexes: `guest(household_id)`, `guest(deleted_at)`, `session(expires_at)`, `audit_log(entity, entity_id)`, `audit_log(at)`, `photo(uploaded_at)`, `seat(seating_unit_id)`.
...
- [ ] Integration: deleting a `seating_unit` or a `seat` that holds an assignment is refused.
- [ ] Integration: a guest may hold one church seat and one party seat, but not two seats in the same venue.
- [ ] Integration: a `seat` or `seat_assignment` whose `venue` disagrees with its parent is rejected.
