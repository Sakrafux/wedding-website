...
6. The same file carries the local development targets — `run` (export `.env`, `go run ./cmd/wedding`) and `test` (`go test ./...`, `gofmt -l .`, `go vet ./...`). Deferred to here rather than written during `E0-02`, so there is one command list instead of two. Nothing in the app parses `.env` itself: the deployed container gets its environment from Compose, so a dotenv dependency would exist for the dev shell alone.
7. Commit an empty `web/dist/.gitkeep` so `go build` works on a clean checkout before the frontend is built, with a comment explaining why it exists.
