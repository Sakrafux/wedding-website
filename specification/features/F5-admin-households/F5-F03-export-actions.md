...
2. Label each with what it is for, not with its filename: "Codes für die Druckerei" and "Gästeliste (alle Daten)". Nobody downloading these thinks in filenames.
3. Next to `codes.csv`, one sentence: this file contains every login code, and it should be deleted from both ends once the cards are printed (`E-OPS-07`). The app cannot enforce that, and a warning nobody sees is the same as no warning.
4. Show the household count next to the codes link, so a truncated or empty file is obvious before it reaches the printer rather than after.
5. Both actions live on the household list page (`F5-F01`). A separate exports page for two links is a page nobody would find.
6. No progress UI. Sixty rows are instantaneous; a spinner would be a lie about the work involved.
