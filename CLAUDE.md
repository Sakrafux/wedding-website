...
- Guest names are **one field**, `guest.name` — the full name as the person is called. No first/last split anywhere (DB, API, UI): every output wants the whole name, and one field copes with a double first name, a missing surname or "Oma Erika". A household's `display_name` is free text too (any group sharing one invitation: "Luki & Paddi"), so a member's name is never derived from it. Accepted cost: nothing sorts by surname.
