...
- Frontend: React + TypeScript + Vite, TanStack Router, TanStack Query, Tailwind v4 (CSS `@theme`, no `tailwind.config.js`), shadcn/ui. Package manager is **pnpm** (via corepack, version pinned in `package.json`) — never npm or yarn. Built `dist/` embedded via `go:embed` — one artifact, FE/BE skew impossible. SPA fallback to `index.html` for non-`/api` paths.
- Frontend tooling: **oxlint** for linting, **prettier** for formatting. **No eslint** — oxlint covers the rules that matter and `strict` + `noUncheckedIndexedAccess` cover most type-aware ones; two linters means two configs that disagree. Versions are pinned by `pnpm-lock.yaml`, not by exact ranges in `package.json`; the Docker build installs with `--frozen-lockfile`. Each dependency's justification lives in `web/README.md` — add one there or do not add it.
...
- Guest names are **one field**, `guest.name` — the full name as the person is called. **Households edit it themselves through the RSVP** (decided 2026-09-20, `F4-B04`); `guest.seeded_name` keeps the name we posted the invitation to and is admin-only, written only by the admin patch. No first/last split anywhere (DB, API, UI): every output wants the whole name, and one field copes with a double first name, a missing surname or "Oma Erika". A household carries **two** free-text names, split 2026-09-19: `household.name` is internal (admin list, search, ordering, `guests.csv`) and `household.addressee` is guest-facing (every greeting, and `codes.csv`, which the cards are printed from) — we file households by surname and address them by first name, and one field could not do both. A member's name is never derived from either. Accepted cost: nothing sorts by surname.
...
- **RSVP deadline: 2027-05-01**, fixed 2026-09-20 and printed on the card. Stored as `app_setting.rsvp_deadline` = `2027-05-01T21:59:59Z` (end of that day in Vienna). Milestones in `07-roadmap.md` are deliberately undated — order is the plan.
