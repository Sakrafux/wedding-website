...
describe("normalizeCode", () => {
  it("produces the canonical form for everything a guest might type", () => {
    // Each of these is a real path: a phone capitalising or not, a paste out of a
    // PDF with a non-breaking space, a word processor's en dash, and a dash typed
    // out of habit — nothing prints one any more, everything still accepts one.
  ...
  it("caps after the dash is gone, so a typed ABC-234 keeps its last digit", () => {
    expect(normalizeCode("ABC-234")).toBe("ABC234");
  });
});
...
describe("CodeInput", () => {
  ...
  // Nothing prints a dash any more, but a guest typing one must not lose a
  // character to it, and a pasted code from an older message must still work.
  it("accepts a typed or pasted dash without losing the last character", async () => {
    const user = userEvent.setup();
    render(<ControlledCodeInput />);
  ...
  it("associates the hint with the field, and adds the error when there is one", () => {
    ...
    expect(field).toHaveAccessibleDescription(/Sechs Zeichen/);
