...
| Path | What |
|---|---|
| `src/routes/` | File-based TanStack Router routes. `__root.tsx` is the shell: the skip link and the outlet, and no `<main>` — each layout renders its own, so the skip link lands *past* the guest navigation. |
| `src/routes/_guest/` | Behind the household login. `_chrome.tsx` is a second pathless layout carrying the navigation; `willkommen.tsx` sits outside it, because the confirmation screen must not offer links that bounce straight back to it. |
| `src/routes/_guest/_chrome/` | The guest pages themselves: start, the content pages, `/mehr`, `/datenschutz` and the RSVP form. |
...
| `src/components/rsvp/` | The RSVP form and its parts. `RSVPForm` takes its data and its save function as **props** and never fetches: that is what lets the guest route and the admin route render the same component against different endpoints. `state.ts` holds the form state and the pure functions over it. |
| `src/components/FormField.tsx`, `HelpPopover.tsx`, `Stepper.tsx` | The guest-facing form furniture: a labelled field with its `?` help popover and its error, and the `−`/`+` number control. Shared, because F4's sheet needs the same three. |
...
| `src/lib/api/rsvp.ts` | The RSVP answer as queries and mutations, in two pairs: the household's own and the admin's by household id. Separate caches, identical shape. |
| `src/components/layout/` | The guest chrome: the bottom bar and the top nav rendered from one entry list (`guestNavigation.ts`), and `InfoSection.tsx`, the heading/prose shape every informational page is built from. |
| `src/lib/wedding.ts` | The wedding date, once. The countdown, the age question and every sentence that spells the date out read it from here — it is still a working assumption, which is exactly why it is one constant. |
...
| `src/assets/hero.jpg`, `hero.webp` | The start page hero. A checked-in **placeholder** at the aspect ratios the layout uses (4:5 mobile, 16:9 desktop); swapping the two files is the whole change when the real photo exists. |
