...
- [x] **RSVP deadline: 2027-05-01**, fixed 2026-09-20 — two and a half months before the wedding, and the date that gets **printed on the card**. Seeded value moved by migration `0005`. Still open: what the site shows after it passes (see below).
- [x] **Guests edit the names of their own people, 2026-09-20.** Seeded and guest-added alike, on the RSVP member card, saved with the rest of the form. `guest.seeded_name` (migration `0006`) keeps the name we invited them under and is shown to us as "Eingeladen als …"; only the admin patch writes it. `F4-B04` / `F4-F04`.
